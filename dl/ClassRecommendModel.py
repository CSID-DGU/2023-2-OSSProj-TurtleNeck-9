...
class Recommend_LSTM(tf.keras.Model):
  ...
  def get_config(self):
    config = {
       ...
       'lstm_weight0':self.lstm.get_weights()[0],
       'lstm_weight1':self.lstm.get_weights()[1],
       'lstm_weight2':self.lstm.get_weights()[2],#그냥 lstm_weights':self.lstm.get_weights()로 한 번에 저장시, 그냥 0,1,2 숫자 하나씩만 array로 저장되기에, 이후 새로 생성된 모델에 이 가중치 불러올 때 오류남.(dense도 마찬가지)
       'dense_weight0':self.dense.get_weights()[0],
       'dense_weight1':self.dense.get_weights()[1],
        }
  ...
  def from_config(cls, config):
    model = cls(
       ...
       lstm_weight0 = config['lstm_weight0'],
       lstm_weight1 = config['lstm_weight1'],
       lstm_weight2 = config['lstm_weight2'],
       dense_weight0 = config['dense_weight0'],
       dense_weight1 = config['dense_weight1']
        )
        # You may need to set other properties or configurations specific to your class here
    return model
  ...
  def BeamSearch(self,taken_classes,num_class=10,beam_size=2):#num_class = 예측할 class 수
    ...
    #encoded_data = self.tokenizer.texts_to_sequences(taken_classes)#훈련 모델 저장 후 불러와서 쓰려면, vocab2idx이용할 수밖에!(tokenizer는 Variable에 제대로 저장 안 되고, 수치형만 config로 저장 잘 되는 듯)
    #print("encoded_data = \n",encoded_data)
...
#    print("next_words_nodes 원소 수(next_words 원소 각각이 노드에 저장되니 개수가 next_words와 동일해야 함!) : ",len(next_words_nodes))
  ...
  def search_max_prob_nodes(self,node,num_path =1):#__call__에서 node = self.tail로 뒀음!!!, 팀원이 path 확률 제일 높은 거 하나만 달라고 해서 num_path=1로 하기! 
