...
file_path = "./dl/data/subject_data.csv"
...
def get_data():
    ...
    id = 3#이 부분 본래 /users/login으로부터 id 받아와야됨. id=1,2,3,4,5
    
    #DB연동해서 로그인된 학생 id로 해당 행 불러오기
    taken_classes, major_of_student = fetch_row_by_id(id)
    ...
    print("기수강 과목 : ", taken_classes)
    print("전공 : ",major_of_student)
    major_taken_classes = [major_of_student]
    major_taken_classes.extend(taken_classes)
    major_taken_classes=[major_taken_classes]

    #DL 연동해서 실제 dl input값(class_recommendation.BeamSearch의 input) 받을 부분(input_for_BeamSearch)

    recommended_classes = class_recommendation.BeamSearch(major_taken_classes, num_class=10, beam_size=2)
